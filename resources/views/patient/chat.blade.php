    ...
    <div style="margin-top:-2rem;position:relative;z-index:10">
      <div class="page-container">
        ...
        <div class="page-card">
          ...
          <form method="POST" action="{{ route('patient.send-message') }}" class="chat-form">
            @csrf
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:14px">
              ...
              <div>
                <label>Subject</label>
                <input type="text" name="subject" placeholder="e.g. Follow-up question" value="{{ old('subject') }}" required>
              ...
              <div style="flex:1;min-width:0">
                ...
                <div style="font-size:.75rem;color:#8898b0">
                  @if($msg->sender_id == Auth::id())
                    To: Dr. {{ $msg->receiver->name ?? 'Unknown' }}
                  @else
                    From: Dr. {{ $msg->sender->name ?? 'Unknown' }}
                  @endif
                </div>
              </div>
            </div>
